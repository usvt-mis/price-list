...
- **Solution implemented**:
  ...
  3. **Post-login redirect to My Records**: After successful authentication, users are automatically redirected to the "My Records" (saved records list) page
     - Login URL includes `post_login=true` parameter: `/.auth/login/aad?post_login_redirect_uri=/?post_login=true`
     - Frontend detects the parameter and redirects authenticated users with valid roles to the list view
     - NoRole users still see the "awaiting assignment" screen and are not redirected
     - URL is cleaned up after processing to prevent redirect loops on refresh
  4. Backend validation in `authExpress.js`:
