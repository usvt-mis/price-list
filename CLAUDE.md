...
- **Features**:
  ...
  - **Backoffice Print Layout Settings**: Administrators can configure print layout settings globally via the backoffice Settings tab
    - Settings include: base font size, logo width, company name font sizes, quotation title font, meta table font, address column width (55-120mm with dynamic column adjustment), attention value width (10-120mm, automatically derived from address column width), left meta value padding (0-6mm for spacing between labels and values), line table font sizes, footer note font, totals font, remark font, signature font, document footer font, certification logos offset, company block offset (horizontal/vertical positioning), attention/tel block offset (horizontal/vertical positioning), totals offset (horizontal positioning), signature block margins
    - **Dynamic Meta Table Column Adjustment**: When address column width increases beyond base (77mm), other columns proportionally shrink (label: 18→14mm, midLabel: 18→15mm, midValue/attentionValue: 34→10mm, rightLabel: 19→14mm, rightValue: 26→19mm) to maintain 183mm total width. The Attention field width automatically adjusts inversely to address column width changes.
    - **Attention/Tel Block Positioning**: attentionTelBlockOffsetXMm (-40 to +20mm horizontal), attentionTelBlockOffsetYMm (±10mm vertical) for precise Attention and Tel field positioning inside the meta table
