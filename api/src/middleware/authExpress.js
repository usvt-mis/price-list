 ...
 * Converted from Azure Functions pattern to Express-compatible middleware
 *
 * This module provides:
 * - requireAuth: Validates Azure AD identity (Express middleware)
 * - requireRole: Validates specific roles (Express middleware factory)
 * - getUserEffectiveRole: Get user's role from database
 * - isExecutive, isSales: Role check helpers
 *
 * Local Development Bypass:
 * - When running locally, authentication is bypassed
 * - Mock user with configurable role is returned
 */
...
/**
 * Factory function to create middleware that requires specific roles
 * @param {...string} allowedRoles - Roles that are allowed (e.g., 'PriceListExecutive')
...
function requireRole(...allowedRoles) {
  return async (req, res, next) => {
    ...
    function checkRole(req, res, next) {
      ...
      if (!hasRole) {
        logger.warn('AUTH', 'ForbiddenInsufficientRole', `User lacks required role`, {
          userEmail: req.user.userDetails,
