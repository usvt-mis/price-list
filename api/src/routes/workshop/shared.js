/**
 * Workshop Shared Calculations API Route (Express)
 * Handles sharing of workshop-specific calculations
...
const { fetchWorkshopCalculationById } = require('./calculations');
...
/**
 * POST /api/workshop/shared/:saveId/share
  ...
  try {
    ...
    if (!user) {
      scopedLogger.warn('AUTH', 'AuthenticationRequired', 'Authentication required for generating share token', {
        serverContext: { endpoint: '/api/workshop/shared/:saveId/share' }
      });
    ...
    scopedLogger.info('BUSINESS', 'ShareTokenGenerationStart', `Generating share token for workshop calculation: ${saveId}`, {
      userEmail,
      userRole,
      serverContext: { endpoint: '/api/workshop/shared/:saveId/share', saveId }
    });
    ...
    if (existing.recordset[0].ShareToken) {
      ...
      timer.stop('BUSINESS', 'ShareTokenRetrieved', `Existing share token returned for saveId: ${saveId}`, {
        userEmail,
        userRole,
        serverContext: { endpoint: '/api/workshop/shared/:saveId/share', saveId, existingToken: true }
      });
    ...
    timer.stop('BUSINESS', 'ShareTokenGenerated', `New share token generated for saveId: ${saveId}`, {
      userEmail,
      userRole,
      serverContext: { endpoint: '/api/workshop/shared/:saveId/share', saveId, shareToken }
    });
...
router.get('/:token', async (req, res, next) => {
  ...
  try {
    ...
    scopedLogger.info('BUSINESS', 'SharedCalculationAccess', `Accessing shared workshop calculation via token`, {
      serverContext: { endpoint: '/api/workshop/shared/:token', tokenPrefix: token.substring(0, 8) }
    });
    ...
    timer.stop('BUSINESS', 'SharedCalculationAccessed', `Shared workshop calculation accessed: ${result.runNumber}`, {
      serverContext: { endpoint: '/api/workshop/shared/:token', runNumber: result.runNumber, saveId }
    });
    ...
    scopedLogger.error('BUSINESS', 'SharedCalculationAccessError', 'Failed to access shared workshop calculation', {
      error: e,
      serverContext: { endpoint: '/api/workshop/shared/:token' }
    });
    next(e);
  } finally {
    scopedLogger.release();
