/**
 * Workshop Saved Calculations API Route (Express)
 * Handles CRUD operations for workshop-specific calculations
 */
...
/**
 * POST /api/workshop/calculations
 * Create new workshop saved calculation
...
router.post('/', async (req, res, next) => {
  ...
  try {
    ...
    scopedLogger.info('BUSINESS', 'WorkshopCalculationSaveStart', `Creating workshop calculation for user: ${userEmail}`, {
      userEmail,
      userRole,
      serverContext: { endpoint: '/api/workshop/calculations', method: 'POST' }
    });
    ...
    if (!branchId || !motorTypeId || salesProfitPct === undefined || travelKm === undefined) {
      scopedLogger.warn('BUSINESS', 'WorkshopCalculationSaveValidationFailed', 'Missing required fields', {
        userEmail,
        userRole,
    ...
    if (!Array.isArray(jobs) || !Array.isArray(materials)) {
      scopedLogger.warn('BUSINESS', 'WorkshopCalculationSaveValidationFailed', 'Jobs and materials must be arrays', {
        userEmail,
        userRole
      });
    ...
    if (branchCheck.recordset.length === 0) {
      ...
      scopedLogger.warn('BUSINESS', 'WorkshopCalculationSaveValidationFailed', 'Branch not found', {
        userEmail,
        userRole,
    ...
    if (motorTypeCheck.recordset.length === 0) {
      ...
      scopedLogger.warn('BUSINESS', 'WorkshopCalculationSaveValidationFailed', 'Motor type not found', {
        userEmail,
        userRole,
    ...
    if (jobs.length > 0) {
      ...
      if (jobIds.length > 0) {
        ...
        if (invalidJobIds.length > 0) {
          ...
          scopedLogger.warn('BUSINESS', 'WorkshopCalculationSaveValidationFailed', 'Invalid job IDs', {
            userEmail,
            userRole,
    ...
    if (materials.length > 0) {
      ...
      if (materialIds.length > 0) {
        ...
        if (invalidMaterialIds.length > 0) {
          ...
          scopedLogger.warn('BUSINESS', 'WorkshopCalculationSaveValidationFailed', 'Invalid or inactive material IDs', {
            userEmail,
            userRole,
    ...
    try {
      // Get next run number (WKS prefix for workshop)
      ...
      const runNumberResult = await requestRunNumber.output("runNumber", sql.NVarChar(12))
        .execute("GetNextWorkshopRunNumber");
      ...
      const result = await fetchWorkshopCalculationById(pool, saveId);

      timer.stop('BUSINESS', 'WorkshopCalculationSaved', `Workshop calculation ${runNumber} created successfully`, {
        userEmail,
        userRole,
        serverContext: { endpoint: '/api/workshop/calculations', runNumber, saveId, jobCount: jobs.length, materialCount: materials.length }
      });
      ...
      const enhancedError = new Error(
        err.message || 'Failed to save workshop calculation'
      );
    ...
    if (e.statusCode === 401) {
      scopedLogger.warn('AUTH', 'AuthenticationRequired', 'Authentication required for saving workshop calculation', {
        serverContext: { endpoint: '/api/workshop/calculations' }
      });
    ...
    if (e.userMessage) {
      scopedLogger.error('BUSINESS', 'WorkshopCalculationSaveFailed', `Failed to create workshop calculation: ${e.message}`, {
        error: e.originalError || e,
        serverContext: e.context || { endpoint: '/api/workshop/calculations' }
      });
    ...
    // Handle other errors
    scopedLogger.error('BUSINESS', 'WorkshopCalculationSaveFailed', 'Failed to create workshop calculation', {
      error: e,
      serverContext: { endpoint: '/api/workshop/calculations' }
    });
    next(e);
...
/**
 * GET /api/workshop/calculations
 * List workshop saved records (role-filtered)
...
router.get('/', async (req, res, next) => {
  try {
    ...
    console.log(`User ${userEmail} listing workshop calculations (Executive: ${isExecutive})`);
    ...
    const r = await pool.request()
      ...
      .query(`
        ...
        LEFT JOIN WorkshopSavedCalculationJobs scj ON sc.SaveId = scj.SaveId
        LEFT JOIN WorkshopSavedCalculationMaterials scm ON sc.SaveId = scm.SaveId
        ${whereClause}
...
/**
 * GET /api/workshop/calculations/:id
 * Get single workshop saved calculation by ID
 * Requires: Authentication
 */
router.get('/:id', async (req, res, next) => {
  try {
    ...
    console.log(`User ${user.userDetails} fetching workshop calculation: ${saveId}`);
...
/**
 * PUT /api/workshop/calculations/:id
 * Update workshop saved calculation (creator only)
 * Requires: Authentication
 */
router.put('/:id', async (req, res, next) => {
  try {
    ...
    console.log(`User ${userEmail} updating workshop calculation: ${saveId}`);
    ...
    try {
      ...
      await new sql.Request(transaction)
        .input('saveId', sql.Int, saveId)
        .query('DELETE FROM WorkshopSavedCalculationJobs WHERE SaveId = @saveId');
      ...
      await new sql.Request(transaction)
        .input('saveId', sql.Int, saveId)
        .query('DELETE FROM WorkshopSavedCalculationMaterials WHERE SaveId = @saveId');
      ...
      console.log(`Updated workshop calculation: ${saveId}`);
      res.status(200).json(result);
      ...
      const enhancedError = new Error(err.message || 'Failed to update workshop calculation');
      enhancedError.originalError = err;
      enhancedError.context = errorDetail;
      enhancedError.userMessage = getUserFriendlyErrorMessage(err);
      throw enhancedError;
...
/**
 * DELETE /api/workshop/calculations/:id
 * Delete workshop saved calculation (creator only, or executive)
 * Requires: Authentication
 */
...
/**
 * POST /api/workshop/calculations/shared/:saveId/share
 * Generate share token for a workshop saved calculation
 * Requires: Authentication (applied at server level)
 */
  ...
  try {
    ...
    if (!user) {
      scopedLogger.warn('AUTH', 'AuthenticationRequired', 'Authentication required for generating share token', {
        serverContext: { endpoint: '/api/workshop/calculations/shared/:saveId/share' }
      });
      return res.status(401).json({ error: 'Authentication required' });
    }
    ...
    scopedLogger.info('BUSINESS', 'ShareTokenGenerationStart', `Generating share token for workshop calculation: ${saveId}`, {
      userEmail,
      userRole,
      serverContext: { endpoint: '/api/workshop/calculations/shared/:saveId/share', saveId }
    });
    ...
    const existing = await pool.request()
      .input('saveId', sql.Int, saveId)
      .query('SELECT CreatorEmail, IsActive, ShareToken FROM WorkshopSavedCalculations WHERE SaveId = @saveId');

    if (existing.recordset.length === 0) {
      scopedLogger.warn('BUSINESS', 'ShareTokenGenerationFailed', 'Workshop calculation not found', {
        userEmail,
        userRole,
        serverContext: { saveId }
      });
    ...
    if (existing.recordset[0].ShareToken) {
      const shareUrl = `${getBaseURL(req)}/workshop.html?share=${existing.recordset[0].ShareToken}`;
      timer.stop('BUSINESS', 'ShareTokenRetrieved', `Existing share token returned for saveId: ${saveId}`, {
        userEmail,
        userRole,
        serverContext: { endpoint: '/api/workshop/calculations/shared/:saveId/share', saveId, existingToken: true }
      });
    ...
    await pool.request()
      .input('saveId', sql.Int, saveId)
      .input('shareToken', sql.NVarChar(36), shareToken)
      .query('UPDATE WorkshopSavedCalculations SET ShareToken = @shareToken WHERE SaveId = @saveId');

    const shareUrl = `${getBaseURL(req)}/workshop.html?share=${shareToken}`;

    timer.stop('BUSINESS', 'ShareTokenGenerated', `New share token generated for saveId: ${saveId}`, {
      userEmail,
      userRole,
      serverContext: { endpoint: '/api/workshop/calculations/shared/:saveId/share', saveId, shareToken }
    });
...
/**
 * GET /api/workshop/calculations/shared/:token
 * Access shared workshop record (PUBLIC - no auth required)
 */
router.get('/shared/:token', async (req, res, next) => {
  ...
  try {
    ...
    scopedLogger.info('BUSINESS', 'SharedCalculationAccess', `Accessing shared workshop calculation via token`, {
      serverContext: { endpoint: '/api/workshop/calculations/shared/:token', tokenPrefix: token.substring(0, 8) }
    });
    ...
    const r = await pool.request()
      ...
      .query('SELECT SaveId FROM WorkshopSavedCalculations WHERE ShareToken = @shareToken AND IsActive = 1');

    if (r.recordset.length === 0) {
      scopedLogger.warn('BUSINESS', 'SharedCalculationNotFound', 'Shared workshop calculation not found or has been deleted', {
    ...
    if (!result) {
      scopedLogger.warn('BUSINESS', 'SharedCalculationFetchFailed', 'Failed to fetch shared workshop calculation', {
        serverContext: { saveId }
      });
    ...
    timer.stop('BUSINESS', 'SharedCalculationAccessed', `Shared workshop calculation accessed: ${result.runNumber}`, {
      serverContext: { endpoint: '/api/workshop/calculations/shared/:token', runNumber: result.runNumber, saveId }
    });
    ...
    scopedLogger.error('BUSINESS', 'SharedCalculationAccessError', 'Failed to access shared workshop calculation', {
      error: e,
      serverContext: { endpoint: '/api/workshop/calculations/shared/:token' }
    });
    next(e);
  } finally {
    scopedLogger.release();
...
/**
 * Helper function to fetch a complete workshop calculation with all related data
 */
async function fetchWorkshopCalculationById(pool, saveId) {
  const r = await pool.request()
    .input('saveId', sql.Int, saveId)
    .query(`
      SELECT sc.SaveId, sc.RunNumber, sc.CreatorName, sc.CreatorEmail,
  ...
  const jobsResult = await pool.request()
    ...
    .query(`
      ...
      FROM WorkshopSavedCalculationJobs
      WHERE SaveId = @saveId
      ORDER BY SortOrder
    `);
...
module.exports.fetchWorkshopCalculationById = fetchWorkshopCalculationById;
