/**
 * Onsite Calculator - Saved Records Sharing Module
 * Handles sharing and shared record loading for onsite calculations
 */
...
export async function shareRecord(saveId, existingShareToken) {
  const shareUrl = existingShareToken
    ? `${window.location.origin}/onsite.html?share=${existingShareToken}`
    : null;
  ...
  try {
    const response = await fetchWithAuth(`/api/onsite/shared/${saveId}/share`, {
      method: 'POST'
    });
...
/**
 * Load shared onsite record from URL
 * @param {string} shareToken - Share token
 */
export async function loadSharedRecord(shareToken) {
  try {
    const record = await fetchJson(`/api/onsite/shared/${shareToken}`);
    setViewOnly(true);
...
/**
 * View a saved onsite record (read-only)
 * @param {number} saveId - Record ID
 */
...
/**
 * Batch delete selected onsite records
 */
