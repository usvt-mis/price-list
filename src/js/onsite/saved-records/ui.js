/**
 * Onsite Calculator - Saved Records UI Module
 * Handles rendering and UI interactions for saved onsite records
 */

import { el, fmt, formatDate, showView } from '../../core/utils.js';
...
/**
 * Render records grid in card view (onsite-specific, no calculator type badge)
...
/**
 * Render records as a table list view (onsite-specific)
 * @param {Array} records - Records to render
 */
...
/**
 * Display record detail in read-only mode (onsite-specific)
...
export function displayRecordDetail(record) {
  ...
  // Build detail HTML (onsite-specific includes Scope, Priority Level, Site Access)
  const detailHtml = `
      ...
      <div class="space-y-4">
        <div class="rounded-xl border border-slate-200 p-4">
          ...
          <div class="space-y-2 text-sm">
            ...
            ${record.scope ? `<div><strong>Scope:</strong> ${record.scope}</div>` : ''}
            ${record.priorityLevel ? `<div><strong>Priority Level:</strong> ${record.priorityLevel}</div>` : ''}
            ${record.siteAccess ? `<div><strong>Site Access:</strong> ${record.siteAccess}</div>` : ''}
          </div>
        </div>
        <div class="rounded-xl border border-slate-200 p-4">
