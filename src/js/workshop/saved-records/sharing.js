/**
 * Workshop Calculator - Saved Records Sharing Module
 * Handles sharing and shared record loading for workshop calculations
 */
...
export async function shareRecord(saveId, existingShareToken) {
  const shareUrl = existingShareToken
    ? `${window.location.origin}/workshop.html?share=${existingShareToken}`
    : null;
  ...
  try {
    const response = await fetchWithAuth(`/api/workshop/shared/${saveId}/share`, {
      method: 'POST'
    });
...
/**
 * Load shared workshop record from URL
 * @param {string} shareToken - Share token
 */
export async function loadSharedRecord(shareToken) {
  try {
    const record = await fetchJson(`/api/workshop/shared/${shareToken}`);
    setViewOnly(true);
...
/**
 * View a saved workshop record (read-only)
 * @param {number} saveId - Record ID
 */
export async function viewRecord(saveId) {
  try {
    const record = await fetchJson(`/api/workshop/calculations/${saveId}`);
...
/**
 * Delete a saved workshop record
 * @param {number} saveId - Record ID
...
export async function deleteRecord(saveId, runNumber) {
  ...
  try {
    ...
    const response = await fetchWithAuth(`/api/workshop/calculations/${saveId}`, requestOptions);
...
/**
 * Batch delete selected workshop records
 */
export async function bulkDeleteRecords() {
  ...
  for (let i = 0; i < saveIds.length; i++) {
    ...
    try {
      const response = await fetchWithAuth(`/api/workshop/calculations/${saveId}`, {
        method: 'DELETE'
      });
